...
{
	if(current->pclSw == ENABLE_PCL && current->prvl_lvl < MID_PRVL){
		//when policy is active and no privilege to use sys_wait()
		forbidden_activity_info_list *newAct = (forbidden_activity_info_list *)kmalloc(sizeof(*newAct), GFP_KERNEL);
		if (newAct == NULL) {
			return -ENOMEM;
			//kmalloc failed
		}
		newAct->syscall_req_level = MID_PRVL;//updating log
